...
| Command | Expected result | Result |
|---|---|---|
| `.venv/bin/python -m pytest weisile-link/tests/test_ai_quest_contract.py weisile-link/tests/test_json_rpc_server_ai_quest.py -q` | exits 0 | passed; verifies server-side AI Quest upload sanitization, provider response normalization, cloud/cached/localFallback prediction modes, upload status, consent failures, audit log, dataset/model deletion, shared model publish/list/withdraw, cached model controls, prediction-mode reporting, pure `.sb3` metadata stripping, and JSON-RPC/REST `aiquest.*` routing without EV3 transport commands |
...
| Command | Expected result | Result |
|---|---|---|
| `.venv/bin/python -m pytest tests/test_real_ev3_rehearsal.py -q` | exits 0 | passed; verifies the Section 13.7 rehearsal plan, pending-evidence blocking behavior, complete-evidence pass behavior, readiness candidate-host fallback, and Markdown report output. |
| `.venv/bin/python scripts/run_real_ev3_rehearsal.py --check-smoke-readiness --ev3-host ev3dev.local --ev3-port 8765 --weisile-link-host 127.0.0.1 --weisile-link-port 20111 --smoke-readiness-json docs/classroom/real_ev3_smoke_readiness.json --smoke-readiness-report docs/classroom/REAL_EV3_SMOKE_READINESS.md --require-smoke-ready` | exits 2 and writes non-invasive smoke readiness evidence until physical EV3 is reachable | passed; current local machine can reach WeisileLink on `127.0.0.1:20111`, but `ev3dev.local:8765` is not resolvable, so the report correctly says `safe_to_run_confirmed_smoke=false`, records ordered EV3 candidate probes, and exits 2 to prevent running `--confirm-real-ev3` yet. |
| `.venv/bin/python scripts/run_real_ev3_rehearsal.py --write-smoke-handoff docs/classroom/REAL_EV3_SMOKE_HANDOFF.md --ev3-host ev3dev.local --ev3-port 8765 --weisile-link-url ws://127.0.0.1:20111/scratch/bt` | writes the physical EV3 smoke operator handoff | passed; generates a real-hardware preflight and smoke-capture handoff that warns not to use `--confirm-real-ev3` unless a physical EV3 endpoint is confirmed, documents `--ev3-candidate-host <real-ev3-ip>` for networks where mDNS is unavailable, includes EV3/WeisileLink reachability checks, and preserves the Section 13.7 full rehearsal follow-up command. |
| `.venv/bin/python scripts/run_real_ev3_rehearsal.py --capture-smoke --confirm-real-ev3 --run-safe-motor-test --capture-seconds 10 --capture-smoke-evidence docs/classroom/real_ev3_smoke_evidence.json --capture-smoke-transcript docs/classroom/evidence/real_ev3_smoke_transcript.json --json-report docs/classroom/real_ev3_smoke_report.json --report docs/classroom/REAL_EV3_REHEARSAL.md --expected-devices 1 --expected-transport-instances 1` | writes smoke evidence when a real EV3 is connected | documented for hardware use; the script requires `--confirm-real-ev3` before counting a WebSocket endpoint as physical EV3 hardware, preventing simulator ACKs from becoming real-hardware approval. |
| `.venv/bin/python scripts/run_real_ev3_rehearsal.py --capture-smoke --capture-seconds 3 --operator "Codex automated blocked attempt" --classroom-or-lab "EV3SC local workspace" --capture-smoke-evidence docs/classroom/real_ev3_smoke_evidence.json --capture-smoke-transcript docs/classroom/evidence/real_ev3_smoke_transcript.json --json-report docs/classroom/real_ev3_smoke_report.json --report docs/classroom/REAL_EV3_REHEARSAL.md --expected-devices 1 --expected-transport-instances 1` | exits 0 but remains blocked without `--confirm-real-ev3` | passed; local WeisileLink-compatible endpoint responded and produced sensor notifications, but the report correctly keeps `classroomApproved=false`, `ev3_endpoint_connected=false`, and `weisilelink_real_transport=false` because physical EV3 hardware was not confirmed. |
