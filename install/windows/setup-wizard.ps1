...
function New-VsleEv3ExternalInstallRunner {
    ...
    $runnerLines = @(
        ...
        '}',
        'Write-VsleEv3RunnerStatus -Summary "EV3 Server install runner is waiting for password input." -Evidence "The external PowerShell window may ask for SSH login first. After SSH connects, the remote EV3 sudo password prompt is visible in that same window. The wizard auto-refreshes this status, and Retry can still load the latest result."',
        'try {',
        "    Import-Module $moduleLiteral -Force -DisableNameChecking",
        '    Write-Host "VSLE EV3 Server install is running."',
        '    Write-Host "Windows OpenSSH may request the SSH login password first."',
        '    Write-Host "After SSH connects, the EV3 will show one visible remote sudo password prompt."',
        '    Write-Host "Press Enter at that remote prompt to use the ev3dev default password."',
        '    Write-Host "Do not share a screenshot while typing a visible password."',
        '    Write-Host "Do not close this window until it prints a final status."',
        '    $plan = Get-Content -Path $planPath -Raw | ConvertFrom-Json',
        '    Write-VsleEv3RunnerStatus -Summary "EV3 Server install runner is executing SSH/SCP commands." -Evidence "The external PowerShell window may be waiting at an OpenSSH host-key, yes/no, SSH login password prompt, or remote EV3 sudo password prompt. If it asks Are you sure you want to continue connecting, type yes and press Enter. If OpenSSH asks Password, type the EV3 SSH password; characters may not appear. If the remote EV3 sudo prompt appears, type the EV3 robot password visibly or press Enter for the default."',
        '    $statusCallback = {',
        '        param($event)',
        ...
        '            "This native OpenSSH/SCP command can wait for host-key confirmation, the EV3 SSH password, or the remote EV3 sudo password prompt. If the external window asks Are you sure you want to continue connecting, type yes and press Enter. If OpenSSH asks Password, type the EV3 SSH password; characters may not appear. If the remote EV3 sudo prompt appears, type the EV3 robot password visibly or press Enter for the default."',
        ...
        '    }',
        '    $result = Invoke-VsleEv3ServerInstall -Plan $plan -ConfirmEv3Install -RunSshCommands -StatusUpdateScript $statusCallback',
        '} catch {',
