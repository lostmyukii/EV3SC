/* global __SCRATCH_AI_ASSET_HOST__ */
...
const DEFAULT_SCRATCH_ASSET_HOST = 'https://cdn.assets.scratch.mit.edu';

const trimTrailingSlash = value => String(value || '').replace(/\/+$/, '');

const getConfiguredScratchAssetHost = () => {
    const configuredHost = typeof __SCRATCH_AI_ASSET_HOST__ === 'undefined' ? '' : __SCRATCH_AI_ASSET_HOST__;
    if (configuredHost === 'same-origin' && typeof window === 'object') {
        return window.location.origin;
    }
    if (configuredHost) {
        return trimTrailingSlash(configuredHost);
    }
    if (typeof window === 'object' && window.SCRATCH_ASSET_HOST) {
        return trimTrailingSlash(window.SCRATCH_ASSET_HOST);
    }
    return DEFAULT_SCRATCH_ASSET_HOST;
};

const getScratchAssetUri = md5ext => (
    `${getConfiguredScratchAssetHost()}/internalapi/asset/${md5ext}/get/`
);
...
const getItemIcons = function (item) {
    ...
    if (item.assetId && item.dataFormat) {
        return {
            ...
            assetServiceUri: getScratchAssetUri(`${item.assetId}.${item.dataFormat}`)
        };
    }
    ...
    if (md5ext) {
        ...
        return {
            ...
            assetServiceUri: getScratchAssetUri(md5ext)
        };
    }
};
