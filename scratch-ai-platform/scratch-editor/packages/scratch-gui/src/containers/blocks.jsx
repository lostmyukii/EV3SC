...
import {AI_LOGIC_WORKSPACE_FOCUS_EVENT} from '../lib/ai/workspace-path-locator';
...
class Blocks extends React.Component {
    constructor (props) {
        ...
        bindAll(this, [
            ...
            'clearAILogicWorkspaceFocus',
            'focusAILogicWorkspaceBlock',
            'handleAILogicWorkspaceFocus',
        ...
        this.aiLogicFocusBlockId = null;
        this.aiLogicFocusTimeout = null;
    ...
    attachVM () {
        ...
        this.props.vm.addListener(AI_LOGIC_WORKSPACE_FOCUS_EVENT, this.handleAILogicWorkspaceFocus);
    ...
    detachVM () {
        ...
        this.props.vm.removeListener(AI_LOGIC_WORKSPACE_FOCUS_EVENT, this.handleAILogicWorkspaceFocus);
        ...
        this.clearAILogicWorkspaceFocus();
    }
    ...
    clearAILogicWorkspaceFocus () {
        if (this.aiLogicFocusTimeout) {
            clearTimeout(this.aiLogicFocusTimeout);
            this.aiLogicFocusTimeout = null;
        }
        if (this.workspace && typeof this.workspace.highlightBlock === 'function') {
            this.workspace.highlightBlock(null);
        }
        if (!this.aiLogicFocusBlockId) return;

        const block = this.workspace && this.workspace.getBlockById(this.aiLogicFocusBlockId);
        if (block) {
            this.ScratchBlocks.glowStack(this.aiLogicFocusBlockId, false);
        }
        this.aiLogicFocusBlockId = null;
    }
    focusAILogicWorkspaceBlock (location) {
        if (!location || !location.blockId || !this.workspace) return;
        if (
            location.targetId &&
            this.props.vm.editingTarget &&
            this.props.vm.editingTarget.id !== location.targetId
        ) {
            return;
        }

        const block = this.workspace.getBlockById(location.blockId);
        if (!block) return;

        this.ScratchBlocks.hideChaff();
        if (typeof this.workspace.centerOnBlock === 'function') {
            this.workspace.centerOnBlock(location.blockId, location.type !== 'logicFlow');
        }
        if (typeof this.workspace.highlightBlock === 'function') {
            this.workspace.highlightBlock(location.blockId);
        }
        this.ScratchBlocks.glowStack(location.blockId, true);
        this.aiLogicFocusBlockId = location.blockId;

        const focusManager = this.ScratchBlocks.getFocusManager && this.ScratchBlocks.getFocusManager();
        if (focusManager && typeof focusManager.focusNode === 'function') {
            try {
    ...
    handleAILogicWorkspaceFocus (location) {
        this.clearAILogicWorkspaceFocus();
        if (!location || location.clear) return;

        this.aiLogicFocusTimeout = setTimeout(() => {
            this.aiLogicFocusTimeout = null;
            this.focusAILogicWorkspaceBlock(location);
        }, 0);
    }
    getToolboxXML () {
