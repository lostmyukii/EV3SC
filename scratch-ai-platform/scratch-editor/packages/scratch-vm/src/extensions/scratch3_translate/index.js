...
const {isTranslateExtensionEnabled} = require('../../util/ai-feature-flags');
 ...
 * @type {string}
 */
const configuredServerURL = readEnvString('SCRATCH_AI_TRANSLATE_SERVICE_URL');
const serverURL = configuredServerURL ? configuredServerURL.replace(/\/?$/, '/') : '';
...
class Scratch3TranslateBlocks {
     ...
     */
    getTranslate (args) {
        if (!isTranslateExtensionEnabled() || !serverURL) {
            log.warn('Translate extension is disabled or not configured.');
