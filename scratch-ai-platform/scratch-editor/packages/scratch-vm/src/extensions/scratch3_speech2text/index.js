...
const {isSpeechToTextExtensionEnabled} = require('../../util/ai-feature-flags');
 ...
 * @type {string}
 */
const serverURL = readEnvString('SCRATCH_AI_SPEECH_TO_TEXT_WS_URL');
...
class Scratch3Speech2TextBlocks {
    ...
    _newSocketCallback (resolve, reject) {
        if (!isSpeechToTextExtensionEnabled() || !serverURL) {
            reject(new Error('Speech to Text extension is disabled or not configured.'));
            return;
        }
    ...
    listenAndWait () {
        if (!isSpeechToTextExtensionEnabled() || !serverURL) {
            log.warn('Speech to Text extension is disabled or not configured.');
