...
if (isScratchAIExtensionEnabled()) {
    builtinExtensions.scratchai = () => require('../extensions/scratch3_ai');
}

const externalServiceExtensionFlags = {
    text2speech: isTextToSpeechExtensionEnabled,
    translate: isTranslateExtensionEnabled,
    speech2text: isSpeechToTextExtensionEnabled
};

const isExternalServiceExtensionDisabled = extensionId => (
    Object.prototype.hasOwnProperty.call(externalServiceExtensionFlags, extensionId) &&
    !externalServiceExtensionFlags[extensionId]()
);
...
class ExtensionManager {
    ...
    loadExtensionIdSync (extensionId) {
        if (isExternalServiceExtensionDisabled(extensionId)) {
            log.warn(`Extension ${extensionId} is disabled by classroom external service policy.`);
            return;
        }
    ...
    loadExtensionURL (extensionURL) {
        if (isExternalServiceExtensionDisabled(extensionURL)) {
            log.warn(`Extension ${extensionURL} is disabled by classroom external service policy.`);
